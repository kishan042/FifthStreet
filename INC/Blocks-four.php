<!-- Sass file = _blocks-three-and-four.scss -->

<!-- Blocks-four -->

<div id="<?php echo $Blocks_4_ID ?>" class="container-fluid">
	<h2 class="txt-xs-center pt-40"><?php echo $Blocks_4_Title ?></h2>
	<div class="container overflow-hidden">
		<div class="row flex-no-wrap-4 py-3">
			<div class="col-md-3 flex-col-4-center">
			    <a href="<?php echo $Blocks_4_IMG_Link_1 ?>">
			   		<img class="img-fluid" src="<?php echo $Blocks_4_IMG_1 ?>" alt="<?php echo $Blocks_4_IMG_1_alt ?>">
					<h2 class="h3-alt"><?php echo $Blocks_4_Product_Title_1 ?></h2>
					<p><?php echo $Blocks_4_Description_1 ?></p>
			    </a>
			</div>
			<div class="col-md-3 flex-col-4-center">
			    <a href="<?php echo $Blocks_4_IMG_Link_2 ?>">
					<img class="img-fluid" src="<?php echo $Blocks_4_IMG_2 ?>" alt="<?php echo $Blocks_4_IMG_2_alt ?>">
					<h2 class="h3-alt"><?php echo $Blocks_4_Product_Title_2 ?></h2>
					<p><?php echo $Blocks_4_Description_2 ?></p>
			    </a>
			</div>
			<div class="col-md-3 flex-col-4-center">
			    <a href="<?php echo $Blocks_4_IMG_Link_3 ?>">
					<img class="img-fluid" src="<?php echo $Blocks_4_IMG_3 ?>" alt="<?php echo $Blocks_4_IMG_3_alt ?>">
					<h2 class="h3-alt"><?php echo $Blocks_4_Product_Title_3 ?></h2>
					<p><?php echo $Blocks_4_Description_3 ?></p>
			    </a>
			</div>
			<div class="col-md-3 flex-col-4-center">
			    <a href="<?php echo $Blocks_4_IMG_Link_4 ?>">
					<img class="img-fluid" src="<?php echo $Blocks_4_IMG_4 ?>" alt="<?php echo $Blocks_4_IMG_4_alt ?>">
					<h2 class="h3-alt"><?php echo $Blocks_4_Product_Title_4 ?></h2>
					<p><?php echo $Blocks_4_Description_4 ?></p>
			    </a>
			</div>
		</div>
	</div>
</div>